...
def readCSVandloaddata(csvfilepath,dataFrom):
    ...
    if(dataFrom == 'households'):
        for eachtuple in dftotuple:
            try:
                query = """INSERT INTO households (HSHD_NUM,L,AGE_RANGE,MARITAL,INCOME_RANGE,HOMEOWNER,HSHD_COMPOSITION,HH_SIZE,CHILDREN) VALUES ({}, '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')""".format(
    ...
    if (dataFrom == 'transactions'):
        for eachtuple in dftotuple:
            try:
                query = """INSERT INTO transactions (BASKET_NUM,HSHD_NUM,PURCHASE_,PRODUCT_NUM,SPEND,UNITS,STORE_R,WEEK_NUM,YEAR_NUM) VALUES ({}, {}, '{}', {}, {}, {}, '{}', {}, {})""".format(
                    int(eachtuple.BASKET_NUM), int(eachtuple.HSHD_NUM), eachtuple.PURCHASE_,
                    int(eachtuple.PRODUCT_NUM), int(eachtuple.SPEND), int(eachtuple.UNITS), eachtuple.STORE_R,
    ...
    if (dataFrom == 'products'):
        for eachtuple in dftotuple:
            try:
                query = """INSERT INTO products (PRODUCT_NUM, DEPARTMENT, COMMODITY, BRAND_TY, NATURAL_ORGANIC_FLAG) VALUES ({}, '{}', '{}', '{}', '{}')""".format(
                    int(eachtuple.PRODUCT_NUM), eachtuple.DEPARTMENT, eachtuple.COMMODITY, eachtuple.BRAND_TY,
                    eachtuple.NATURAL_ORGANIC_FLAG)
                cursor.execute(query)
            except Exception as e:  # work on python 3.x
                print('Failed to upload to ftp: ' + str(e))
    conn.commit()
